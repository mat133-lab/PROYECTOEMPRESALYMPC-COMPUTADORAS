...
function buildPedidoPdfHtml(PDO $conn, int $id_pedido): string
{
    ...
    $stmtItems = $conn->prepare("SELECT dp.cantidad, dp.precio_unitario, pr.nombre FROM detalles_pedido dp JOIN productos pr ON pr.id_producto = dp.id_producto WHERE dp.id_pedido = ? ORDER BY dp.id_detalle ASC");
    ...
    $cliente = htmlspecialchars($pedido['usuario'], ENT_QUOTES, 'UTF-8');
    $correo = htmlspecialchars($pedido['correo'], ENT_QUOTES, 'UTF-8');
    $cedula = htmlspecialchars($pedido['cedula'] ?? 'N/A', ENT_QUOTES, 'UTF-8');
...
<body class="pdf-page">
  ...
  <div class="card">
    ...
    <div class="footer">Gracias por confiar en L&M PC Computadoras. Este comprobante se genera automáticamente para su respaldo y seguimiento.</div>
  </div>
...
function generatePedidoPdf(PDO $conn, int $id_pedido, string $outputPath): string
{
    $html = buildPedidoPdfHtml($conn, $id_pedido);
