...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] === UPLOAD_ERR_OK) {
        ...
        $ruta_destino = '../img/' . $nombre_imagen_final;
        
        // Mover el archivo temporal a la carpeta final
        if (!move_uploaded_file($archivo_tmp, $ruta_destino)) {
            die("Error al subir la imagen. Verifica los permisos de la carpeta img.");
        }
    }
    ...
    try {
        ...
        if ($stmt->execute()) {
            ...
            header("Location: dashboard.php?mensaje=guardado_exito");
            exit();
        } else {
